...
def check_product_function(C, ss):
    ...
    while ss:
        ...
        summ += ss.pop(0)
    return C[summ]
def find_pair(c_node, d, mps=False, with_chi=False):
    ...
    if with_chi==True:
        c_node.data = c_node.data/np.linalg.norm(c_node.data)
    for a_index, b_index in list(combinations(c_node.indices, 2)):
        ...
        c_AB = c_temp.reshape(a_index["index"].index_dim*b_index["index"].index_dim, -1)
        c_A = c_temp.reshape(a_index["index"].index_dim, -1)
        c_B = np.transpose(c_A).reshape(b_index["index"].index_dim, -1)
...
def make_tree_with_chi(d, L, C, chi, mps=False):
    sigma_temp = [Index(index_type= "sigma", index_dim=d) for i in range(L)]
    c_indices = [{"place":i, "index":sigma_temp[i]} for i in range(L)]
    c_node = c_Node(data=C, indices = c_indices)
    Tree = []
    ...
    while 4<=len(c_node.indices):
        assert check_c_node(c_node)
        # print(len(c_node.indices), c_node.data.shape)
        a, b = find_pair(c_node, d, mps, with_chi=True)
        # print(len(list(combinations(c_node.indices, 2))))
        assert 4<=len(c_node.indices)
        l = list(range(len(c_node.indices)))
        l[0],l[a["place"]] = l[a["place"]], l[0]
...
def product(corrent_node, Tree, s_num, n_dict):
    assert check_node(corrent_node)
    ...
    if corrent_node.data_type != "diagonal":
        for node in Tree:
            if node.down_l.index_id == corrent_node.up.index_id:
                ...
                # print(corrent_node.up.index_dim)
                # print(node.down_l.index_dim)
                # print(corrent_node.data.shape)
                # print(node.down_r.index_dim)
                # print(node.up.index_dim)
...
def create_random_state(W, L):
    d = 2
    Sz = 0.5*np.array([[1, 0], [0, -1]])
    Sp = np.array([[0, 1], [0, 0]])
    H_hopp = 0.5*(np.kron(Sp, np.transpose(Sp)) + np.kron(np.transpose(Sp), Sp)) 
    H_mag = np.kron(Sz, Sz)
    w = np.random.uniform(low=0, high=W, size=L)
    H = 0
    for i in range(L-1):
        H += np.kron(np.eye(d**i), np.kron(H_hopp+w[i]*H_mag, np.eye(d**(L-2-i))))
    H += 0.5*(np.kron(Sp, np.kron(np.eye(d**(L-2)), Sp.T))+np.kron(Sp.T, np.kron(np.eye(d**(L-2)), Sp))) + w[-1]*np.kron(Sz, np.kron(np.eye(d**(L-2)), Sz))
    (energy,), psi = eigsh(H, k = 1, which="SA")
    return psi

def create_rainbow_state(L):
    d = 2
    Sp = np.array([[0,1], [0,0]])
    H_two = np.kron(Sp, np.transpose(Sp)) + np.kron(np.transpose(Sp), Sp)
    w = [100**(-np.abs(i - L/2+1)) for i in range(L-1)]
    H = 0
    for i in range(L-1):
        H += w[i]*np.kron(np.eye(d**i), np.kron(H_two, np.eye(d**(L-2-i))))
    # assert np.allclose(H, H.T)
    (energy,), psi = eigsh(H, k = 1, which="SA")
    print(energy)
    return psi

def create_free_hopping_state(L):
    d = 2
    Sp = np.array([[0,1], [0,0]])
    H_two = np.kron(Sp, np.transpose(Sp)) + np.kron(np.transpose(Sp), Sp)
    H = 0
    for i in range(L-1):
        H += np.kron(np.eye(d**i), np.kron(H_two, np.eye(d**(L-2-i))))
    # assert np.allclose(H, H.T)
    (energy,), psi = eigsh(H, k = 1, which="SA")
    print(energy)
    return psi
def create_heisenberg_state(L, boundary="open"):
    d = 2
    sz = 0.5*np.array([[1., 0.],[0., -1.]])
    sp = np.array([[0, 1.],[0., 0.]])
    I = np.eye(d**2)
    two_site_h = np.kron(sz, sz)+0.5*(np.kron(sp, sp.T)+np.kron(sp.T, sp))
    H = 0
    for i in range(L-1):
        H = H + np.kron(np.eye(d**i), np.kron(two_site_h, np.eye(d**(L-2-i))))
    if boundary=="periodic":
        H = H + np.kron(sz, np.kron(np.eye(d**(L-2)), sz)) + 0.5*(np.kron(sp, np.kron(np.eye(d**(L-2)), sp.T)) + np.kron(sp.T, np.kron(np.eye(d**(L-2)), sp))) 
    w, v = eigsh(H, k=1)
    print("E/N=", w[0]/L)
    return v[:,0]
def powerset(iterable):
    s = list(iterable)
    return chain.from_iterable(combinations(s, r) for r in range(1, int(len(s)/2)+1))
def bipartite_ent(indices, group, C ):
    l = [i for i, x in enumerate(indices)]
    int_group = [indices.index(i) for i in group]
    int_comp_group = list(set(l) - set(int_group))
    permutation_list = list(int_group) + int_comp_group
    C = np.transpose(C.reshape([2]*len(l)), permutation_list).reshape([2**len(int_group), -1])
    rho = C.T@C
    comp_group = [indices[i] for i in int_comp_group]
    new_indices = comp_group + ["p"+i for i in comp_group]
    return entropy(rho)
class min_info:
    def __init__(self, L, boundary):
        self.L = L
        self.whole_indices = [str(i+1) for i in range(L)]
        with open("entropy_{}_N{}.pkl".format(boundary, str(L)), "rb") as f:
            self.entropy_dict = pickle.load(f)
    def find_min_ent(self,indices):
        min_S = float("inf")
        for group in self.powerset(indices):
            partial_comp_group = tuple(set(indices) - set(group))
            comp_group = tuple(set(self.whole_indices)-set(partial_comp_group))
            comp_group = comp_group if len(comp_group) < len(partial_comp_group) else partial_comp_group
            comp_group = self.sort_tuple(comp_group)
            if self.entropy_dict[group] + self.entropy_dict[comp_group] < min_S:
                min_group = group
                min_partial_comp_group = partial_comp_group
                min_S = self.entropy_dict[group] + self.entropy_dict[comp_group]

        # print(min_S)
        return min_group, self.sort_tuple(min_partial_comp_group)
    ...
    def make_tree(self):
        ...
        while target:
            indices = target.pop()
            if len(indices) > 1:
                group, comp_group = self.find_min_ent(indices)
                node = {"id":str(hash(tuple(indices))), "downs":[group[0] if len(group)==1 else str(hash(group)), comp_group[0] if len(comp_group)==1 else str(hash(comp_group))]}
                hash_dict[str(hash(tuple(indices)))] = str(i)
                i += 1
                target.extend([group, comp_group])
                tree.append(node)
        for node in tree:
            node["id"] = hash_dict[node["id"]]
            indices = []
            for index in node["downs"]:
                if len(index)<3:
                    indices.append("s"+index)
                else:
                    indices.append(hash_dict[index])
            node["downs"] = set(indices)
        for i in range(self.L):
            tree.append({"id":"s"+str(i+1), "downs":set([])})
        return tree
    def powerset(self, iterable):
        s = list(iterable)
        return chain.from_iterable(combinations(s, r) for r in range(1, int(len(s)/2)+1))
    def bipartite_ent(self, indices, group, C ):
        l = [i for i, x in enumerate(indices)]
        int_group = [indices.index(i) for i in group]
        int_comp_group = list(set(l) - set(int_group))
        permutation_list = list(int_group) + int_comp_group
        ...
        new_indices = comp_group + ["p"+i for i in comp_group]
        return entropy(rho)
    def make_ent_dict(self, boundary):
        d = 2
        C =  create_heisenberg_state(self.L, boundary=boundary)
        indices = [str(i+1) for i in range(self.L)]
        entropy_dict = {}
        for group in powerset(indices):
            entropy_dict[group] = bipartite_ent(indices, group, C)
        with open("entropy_{}_N{}.pkl".format(boundary, str(self.L)), "wb") as f:
            pickle.dump(entropy_dict, f)
if __name__ == "__main__":
    d = 2
    L = 8
    # create_rainbow_state(L)
